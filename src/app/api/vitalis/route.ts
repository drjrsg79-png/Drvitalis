...
import { guardarHistorialChat } from '@/lib/db';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
      try {
        await guardarHistorialChat(email, [...conversation, { role: 'assistant', content: reply }]);
      } catch {
        // La respuesta médica no debe perderse si la persistencia falla temporalmente.
      }
    }
